...
import { products } from '@/lib/products'; // 🔥 Catálogo en vivo
      ...
      <div className={`fixed top-0 right-0 h-full w-full max-w-md bg-white shadow-2xl z-[70] transform transition-transform duration-300 ease-out flex flex-col ${isCartOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 scrollbar-thin scrollbar-thumb-neutral-200">
            {items.length === 0 ? (
                ...
                items.map((item) => {
                    // 🔥 BLINDADO NIVEL DIOS: Extraemos el ID a la fuerza si el LocalStorage está roto
                    const extractedProductId = item.productId || (item.id ? item.id.split('-')[0] : null);
                    
                    // Buscamos el producto exacto en vivo desde tu catálogo
                    const liveProduct = products.find((p) => p.id === extractedProductId);
                    
                    // Buscamos si tiene un color específico
                    const colorInfo = liveProduct?.colors?.find((c) => c.name === item.meta?.color);
                    
                    // Prioridad de imágenes absoluta. Ya no hay cajita de fallback.
                    const imageSrc = item.image || item.meta?.image || colorInfo?.image || liveProduct?.thumbnail || "/placeholder.jpg";
                    ...
                    return (
                        <div key={item.id} className="flex gap-4 animate-in fade-in slide-in-from-right-4 border-b border-neutral-50 pb-4 last:border-0">
                            ...
                            <div className="relative w-20 h-20 bg-neutral-100 rounded-md overflow-hidden shrink-0 border border-neutral-200 flex items-center justify-center">
                                {/* 🔥 Forzamos el renderizado de la imagen siempre */}
                                <Image 
                                    src={imageSrc} 
