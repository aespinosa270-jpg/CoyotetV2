 ...
 *  1. `verifyWebhook()` — verifica la firma y devuelve el evento parseado.
 *     Si la firma es inválida → throw WebhookSignatureError.
 *  2. `parseCheckoutCompleted()` — extrae los datos relevantes de un evento
 *     `checkout.session.completed` como un objeto tipado, listo para
 *     consumir por el orquestador.
 *
 * El módulo NO ejecuta side effects (registrar pedido, mandar mensaje, emitir
...
export interface CheckoutCompletedPayload {
  sessionId: string;
  /** Teléfono del cliente, recuperado del metadata. */
  phone: string;
  /** Productos comprados (lista CSV en metadata). */
  productos: string[];
  /** Monto total cobrado en MXN. */
  amountMxn: number;
  /** Método principal usado: card | oxxo | bank_transfer. */
  paymentMethod: string;
  /** Datos fiscales si el cliente pidió factura. */
  factura?: FacturaInfo;
  /** Evento crudo, por si el handler necesita acceder a algo más. */
 ...
 * lo recibió Next.js (sin parseo JSON intermedio). En Next.js App Router se
...
export function verifyWebhook(
  ...
  try {
    ...
    throw new WebhookSignatureError(`Stripe webhook signature inválida: ${message}`);
  }
}
 ...
 *  - Falta el metadata.phone (la sesión no fue creada por el bot).
 *
 * Esto permite usar la función como filtro: si devuelve null, el orquestador
 * ignora el evento.
 */
...
// ── Determinar formaPago para Facturapi ───────────────────────────

/**
 * Mapea el método de pago de Stripe al código `payment_form` del SAT.
 *
 *   01 = efectivo   (OXXO)
