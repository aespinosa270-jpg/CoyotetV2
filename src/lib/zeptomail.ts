...
// ─────────────────────────────────────────────────────────────────────
// HELPER: enviar correo genérico via ZeptoMail
// Lee la respuesta como texto antes de parsear para evitar
// SyntaxError cuando ZeptoMail devuelve un body vacío en errores.
// ─────────────────────────────────────────────────────────────────────
async function enviarCorreo(payload: object, contexto: string) {
  ...
  if (!response.ok) {
    console.error(`❌ ZeptoMail error (${contexto}) status ${response.status}:`, rawText)
    throw new Error(`Fallo al enviar correo (${contexto}): ${response.status} — ${rawText.slice(0, 200)}`)
  }

  const data = rawText ? JSON.parse(rawText) : {}
  console.log(`✅ Correo enviado (${contexto})`)
...
function generarHtmlMembresia(clienteNombre: string, memberId: string, tier: string = "NONE") {
  ...
  <head>
    ...
    <style>
      ...
      .css-card-container { animation: cardShine 4s infinite ease-in-out; transition: all 0.3s ease; }
              ...
              <tr>
                <td align="center">
                  <div style="width: 200px; margin: 0 auto;">
                    <div style="font-size: 42px; font-weight: 900; letter-spacing: 2px; color: #ffffff; text-shadow: 2px 2px 4px rgba(0,0,0,0.8); line-height: 1;">COYOTE</div>
                    ...
                    <table width="100%" cellpadding="0" cellspacing="0" border="0">
                      <tr>
                        <td width="40" align="left">
                          <div style="background-color: #111111; color: #ffffff; font-weight: bold; font-size: 14px; width: 36px; height: 36px; border-radius: 8px; text-align: center; line-height: 36px;">CY</div>
                        </td>
                        ...
                        <td align="right">
                          <div style="background-color: #111111; color: #aaaaaa; font-size: 9px; font-weight: bold; padding: 6px 12px; border-radius: 4px; letter-spacing: 2px;">${config.material}</div>
                        </td>
                      </tr>
                    </table>
                    <table width="100%" cellpadding="0" cellspacing="0" border="0">
                      <tr>
                        <td align="center" style="padding: 40px 0;">
                          <div class="base-text" style="font-size: 60px; font-weight: 900; letter-spacing: 12px; color: ${config.textColor}; background: ${config.mainTextGradient}; -webkit-background-clip: text; -webkit-text-fill-color: transparent; text-shadow: 2px 2px 4px rgba(0,0,0,0.1);">${config.titulo}</div>
                        </td>
                      </tr>
                    </table>
                    <table width="100%" cellpadding="0" cellspacing="0" border="0">
                      <tr>
                        <td align="left">
                          <div style="font-size: 9px; font-weight: 700; color: ${config.subTextColor}; letter-spacing: 2px; text-transform: uppercase; margin-bottom: 5px;">Credencial de Acceso</div>
                          <div style="font-family: 'Courier New', Courier, monospace; font-size: 16px; font-weight: 900; color: ${config.textColor}; letter-spacing: 4px;">MX-${memberId} &bull;&bull;&bull;&bull; ${anio}</div>
                        </td>
                      </tr>
                    </table>
                  </td>
...
export async function sendMembresiaEmail(clienteEmail: string, clienteNombre: string, memberId: string, tier: string = "NONE") {
  ...
  const payload = {
    ...
    htmlbody: generarHtmlMembresia(clienteNombre, memberId, tier),
  }
  try {
    return await enviarCorreo(payload, `membresia-${tier}`)
  } catch (error) {
    console.error("❌ Excepción al enviar correo de membresía:", error)
            ...
            <table width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width: 400px; background-color: #111111; border-radius: 8px; border: 1px solid #222222;">
              <tr>
                <td style="padding: 25px; text-align: center;">
                  <p style="margin: 0 0 15px 0; font-size: 14px; font-weight: 700; color: #ffffff; letter-spacing: 1px;"><span class="yellow-text">✓</span> CATÁLOGO COMPLETO</p>
                  <p style="margin: 0 0 15px 0; font-size: 14px; font-weight: 700; color: #ffffff; letter-spacing: 1px;"><span class="yellow-text">✓</span> LOGÍSTICA INTEGRADA</p>
                  <p style="margin: 0; font-size: 14px; font-weight: 700; color: #ffffff; letter-spacing: 1px;"><span class="yellow-text">✓</span> ATENCIÓN INTELIGENTE</p>
                </td>
              </tr>
            </table>
          </td>
...
export async function sendBienvenidaEmail(clienteEmail: string, clienteNombre: string) {
  ...
  const payload = {
    ...
    htmlbody: generarHtmlBienvenida(clienteNombre),
  }
  try {
    return await enviarCorreo(payload, "bienvenida")
  } catch (error) {
...
export async function sendResetPasswordEmail(clienteEmail: string, clienteNombre: string, resetUrl: string) {
  ...
  try {
    return await enviarCorreo(payload, "reset-password")
  } catch (error) {
