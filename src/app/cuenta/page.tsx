        ...
        <motion.div variants={{hidden:{opacity:0}, show:{opacity:1}}} initial="hidden" animate="show" className="relative z-30 w-full max-w-xl p-12">
            ...
            <motion.div variants={itemVars} className="flex flex-col gap-8">
               {[
                 ...
                 { icon: Zap, title: "Precios de Fábrica", subtitle: "Exclusivo para Socios" },
                 { icon: Ship, title: "Logística Nacional", subtitle: "Envíos gratis en niveles altos" }
               ].map((item, i) => (
              ...
              <motion.div key="login" variants={containerVars} initial="hidden" animate="show" exit="exit" className="w-full relative">
                <motion.div variants={formItemVars} className="mb-14">
                    ...
                    <h1 className="text-5xl font-[1000] uppercase tracking-tighter leading-none italic">
                      <span className="text-white">PORTAL</span> <span className="text-[#FDCB02]">SOCIOS</span>
                    </h1>
                ...
                <form onSubmit={handleSubmit} className="space-y-10">
                  <motion.div variants={formItemVars}>
                    <label className="block text-[11px] font-black uppercase tracking-widest text-neutral-500 mb-2">E-mail B2B</label>
                    <input type="email" name="email" value={formData.email} onChange={handleInputChange} required className="w-full h-12 bg-transparent border-b-2 border-white/20 focus:border-[#FDCB02] outline-none font-bold text-lg text-white transition-colors rounded-none placeholder:text-neutral-800 px-0" placeholder="ceo@empresa.com" />
                  </motion.div>
            ...
            {authMode === 'register' && (
              <motion.div key="register" variants={containerVars} initial="hidden" animate="show" exit="exit" className="w-full">
                 <motion.div variants={formItemVars} className="mb-12">
                    <h1 className="text-5xl font-[1000] uppercase tracking-tighter leading-none italic">
                      <span className="text-white">UNIRSE A LA</span><br/><span className="text-[#FDCB02]">MANADA</span>
                    </h1>
                    <motion.div variants={lineVars} className="h-[4px] bg-[#FDCB02] mt-4 mb-4" />
                    <p className="text-neutral-400 font-bold text-[11px] uppercase tracking-widest leading-relaxed">
                      Crea tu cuenta básica gratis para explorar el inventario.
                    </p>
                </motion.div>
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                  ...
                  <motion.div variants={formItemVars}>
                    <label className="block text-[11px] font-black uppercase tracking-widest text-neutral-500 mb-2">E-mail Corporativo</label>
                  ...
                  <motion.div variants={formItemVars}>
                    <label className="block text-[11px] font-black uppercase tracking-widest text-neutral-500 mb-2">Contraseña (Min 8)</label>
                    <input type="password" name="password" value={formData.password} onChange={handleInputChange} required minLength={8} className="w-full h-12 bg-transparent border-b-2 border-white/20 focus:border-[#FDCB02] outline-none font-bold text-lg text-white transition-colors rounded-none px-0 placeholder:text-neutral-800" placeholder="••••••••"/>
                  </motion.div>
            ...
            {authMode === 'upsell' && (
              <motion.div key="upsell" variants={containerVars} initial="hidden" animate="show" exit="exit" className="w-full">
                 <motion.div variants={formItemVars} className="mb-8">
                    <h1 className="text-5xl font-[1000] uppercase tracking-tighter leading-none italic flex flex-col">
                      <span className="text-white">NIVEL ACTUAL:</span>
                      <span className="text-neutral-500 line-through decoration-[#FDCB02] decoration-4 mt-2">BÁSICO</span>
                    </h1>
                    <motion.div variants={lineVars} className="h-[4px] bg-[#FDCB02] mt-6 mb-4" />
                    
                    <div className="bg-neutral-900/50 border border-white/10 p-4 rounded-sm mb-6">
                      <p className="text-neutral-300 font-bold text-[11px] uppercase tracking-widest leading-relaxed">
                        Tu cuenta ha sido creada. Actualmente tienes el nivel <span className="text-white">Básico (Mortal)</span>, lo que significa que estás viendo precios de menudeo.
                      </p>
                    </div>

                    <div className="bg-[#FDCB02]/10 border border-[#FDCB02]/30 p-4 rounded-sm">
                      <div className="flex items-center gap-2 mb-2">
                        <Crown size={16} className="text-[#FDCB02]" />
                        <span className="text-[#FDCB02] font-[1000] text-[11px] uppercase tracking-[0.2em]">Desbloquea tu potencial B2B</span>
                      </div>
                      <p className="text-neutral-400 font-bold text-[10px] uppercase tracking-widest leading-relaxed">
                        Sube a membresía GOLD, BLACK o ELITE para acceder a <span className="text-white">precios directos de fábrica</span>, envíos gratis y acumular puntos reales.
                      </p>
                    </div>
                 </motion.div>

                 <motion.div variants={formItemVars} className="flex flex-col gap-4 relative z-10 mt-8">
                    <button onClick={() => router.push('/membresia')} className="w-full h-16 bg-[#FDCB02] hover:bg-white text-black font-[1000] text-sm uppercase tracking-[0.2em] flex items-center justify-between px-6 transition-colors duration-300 rounded-none group">
                      <span>VER MEMBRESÍAS</span>
                      <ArrowRight size={20} strokeWidth={2.5} className="group-hover:translate-x-2 transition-transform duration-300"/>
                    </button>
                    
                    <button onClick={() => router.push('/catalogo')} className="w-full h-12 bg-transparent text-neutral-500 hover:text-white font-black text-[10px] uppercase tracking-widest transition-colors duration-300 rounded-none text-center">
                      Continuar al catálogo con precios altos →
                    </button>
                 </motion.div>
              </motion.div>
            )}
