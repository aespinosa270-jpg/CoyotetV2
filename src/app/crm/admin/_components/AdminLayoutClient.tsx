...
// ─── Menu agrupado por tarea (mismas rutas, reorganizadas) ───
type MenuItem = { name: string; icon: any; href: string; submenus?: { name: string; href: string }[]; badgeKey?: string };
...
const menuGroups: MenuGroup[] = [
  {
    ...
    items: [
      ...
      { name: "El Coyote (bot)", icon: Bot, href: "/crm/admin/bot" },
      {
  ...
  },
  {
    group: "Configurar",
    items: [
      { name: "Configuracion", icon: Settings, href: "/crm/admin/configuracion" },
    ],
  },
];
          ...
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }} transition={{ duration: 0.15 }}
            ...
            {!loading && results.map((r) => (
              <button key={r.id} onClick={() => handleSelect(r.href)}
                className="w-full flex items-start gap-3 px-4 py-3 hover:bg-[#1c2026] transition-colors text-left border-b border-[#22272f] last:border-0">
...
export default function AdminLayoutClient({
  children, employee, notifCount,
}: { children: React.ReactNode; employee: Employee; notifCount: number; }) {
    ...
    <div className="flex h-screen bg-[#0a0b0d] text-zinc-100 overflow-hidden selection:bg-amber-400 selection:text-black font-sans">
      ...
      <aside className={`fixed md:static inset-y-0 left-0 z-50 w-64 bg-gradient-to-b from-[#101216] to-[#0a0b0d] border-r border-[#2c323b]
        ...
        <div className="h-16 flex items-center justify-between px-5 border-b border-[#2c323b] shrink-0">
          <h1 className="text-lg font-[1000] uppercase tracking-tighter flex items-center gap-2">
            <span className="text-xl">🐺</span> COYOTE <span className="text-amber-400">CRM</span>
          </h1>
                    ...
                    <div key={item.name}>
                      <button onClick={() => hasSubmenu ? toggleSubmenu(item.name) : (router.push(item.href), setIsMobileOpen(false))}
                        ...
                        <div className="flex items-center gap-3">
                          <item.icon size={15} className={isItemActive ? "text-amber-400" : ""} />
                          {item.name}
                        </div>
                      ...
                      <AnimatePresence initial={false}>
                        {hasSubmenu && isSubmenuOpen && (
                          <motion.div key="sub" initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                            ...
                            <div className="ml-7 mt-1 flex flex-col gap-1 border-l border-[#2c323b] pl-3 py-1">
        ...
        <div className="p-3 border-t border-[#2c323b] shrink-0">
          <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-[#15181d] border border-[#2c323b]">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[12px] font-bold text-zinc-100 truncate">{employee?.name ?? "Admin"}</p>
              <p className="text-[9px] text-zinc-500 uppercase tracking-widest truncate font-bold">{employee ? ROLE_LABEL[employee.role] : "—"}</p>
            </div>
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="h-16 border-b border-[#2c323b] bg-[#0d0d0f]/90 backdrop-blur-md flex items-center justify-between px-4 md:px-6 z-40 sticky top-0 shrink-0">
          <div className="flex items-center gap-4">
            <button className="md:hidden text-zinc-400 hover:text-zinc-100 transition-colors" onClick={() => setIsMobileOpen(true)}><Menu size={22} /></button>
            <GlobalSearch />
          </div>
          <div className="flex items-center gap-4">
            <Link href="/crm/admin/tickets/abiertos" className="relative text-zinc-400 hover:text-amber-400 transition-colors">
              <Bell size={17} />
              {notifCount > 0 && <span className="absolute -top-1.5 -right-1.5 w-4 h-4 bg-rose-500 border-2 border-[#0d0d0f] rounded-full flex items-center justify-center text-[7px] font-black text-white">{notifCount > 9 ? "9+" : notifCount}</span>}
            </Link>
            ...
            <div ref={profileRef} className="relative">
              <button onClick={() => setIsProfileOpen((p) => !p)} className="flex items-center gap-3 hover:opacity-80 transition-opacity">
                <div className="text-right hidden md:block">
                  <p className="text-[11px] font-bold text-zinc-100 uppercase tracking-wider leading-none">{employee?.name?.split(" ")[0] ?? "Admin"}</p>
                  <p className="text-[9px] text-zinc-500 font-bold mt-0.5">{employee ? ROLE_LABEL[employee.role] : "—"}</p>
                </div>
              ...
              <AnimatePresence>
                {isProfileOpen && (
                  <motion.div key="profile-dd" initial={{ opacity: 0, y: 8, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 8, scale: 0.96 }} transition={{ duration: 0.12 }}
                    className="absolute right-0 mt-3 w-52 bg-[#15181d] border border-[#2c323b] rounded-2xl shadow-xl py-2 overflow-hidden z-50">
                    <div className="px-4 py-3 border-b border-[#22272f] mb-1">
                      <p className="text-xs font-bold text-zinc-100">{employee?.name}</p>
                    ...
                    <Link href="/crm/admin/tickets/abiertos" onClick={() => setIsProfileOpen(false)} className="flex items-center justify-between px-4 py-2.5 text-[11px] text-zinc-400 hover:text-zinc-100 hover:bg-[#1c2026] font-bold uppercase tracking-widest transition-colors">
                      <span className="flex items-center gap-3"><AlertTriangle size={13} className="text-rose-400" /> Tickets Urgentes</span>
                      {notifCount > 0 && <span className="text-[8px] bg-rose-500/15 text-rose-400 border border-rose-500/30 px-1.5 py-0.5 rounded-full font-black">{notifCount}</span>}
                    </Link>
        ...
        {/* CONTENIDO — fondo claro por ahora para que las paginas hijas se vean bien */}
        <div className="flex-1 overflow-y-auto p-4 md:p-8 bg-[#F8F9FA] text-black
          [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:bg-zinc-300 [&::-webkit-scrollbar-thumb]:rounded-full">
          {children}
        </div>
      </main>
    </div>
