...
import {
  ...
  TrendingUp, ShieldCheck, Package, X,
} from "lucide-react";
...
function ChatPane({ phone, resumen, onBack }: {
  ...
  async function send() {
    ...
    const optimistic: MensajeHistorial = {
      role: "assistant", content: t, timestamp: new Date().toISOString(), status: "sent",
    };
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-[#0a0a0b] relative">
        ...
        <div className="relative z-10 px-4 pt-3 pb-4 border-t border-zinc-800/70 bg-[#0d0d0f]/75 backdrop-blur-md">
          ...
          <div className="flex items-end gap-2 bg-zinc-900 border border-zinc-800 rounded-2xl pl-3 pr-2 py-2 focus-within:border-amber-400/50 focus-within:ring-2 focus-within:ring-amber-400/10 transition">
            ...
            />
            <button title="Adjuntar (usa el panel inferior)" className="w-9 h-9 rounded-lg grid place-items-center text-zinc-500 hover:text-zinc-300 transition"><Paperclip className="w-[18px] h-[18px]" /></button>
            <button onClick={send} disabled={sending || !text.trim()} className="w-10 h-10 rounded-xl bg-amber-400 hover:bg-amber-300 disabled:bg-zinc-700 disabled:text-zinc-500 text-black grid place-items-center transition shadow-[0_0_18px_rgba(251,191,36,0.25)] disabled:shadow-none active:scale-95">
