...
export default async function AdminDashboardPage() {
  // 1. PROTOCOLO DE SEGURIDAD MASTER
  ...
  // 2. EXTRACCIÓN SÍNCRONA DE DATOS (POSTGRESQL)
  ...
  // 3. MOTOR DE INTELIGENCIA COMERCIAL
  ...
  // Métricas de Pipeline Logístico
      ...
      <main className="relative z-10 max-w-[1600px] mx-auto px-6 py-10 lg:py-16 animate-in fade-in slide-in-from-bottom-12 duration-1000">
        ...
        <header className="flex flex-col xl:flex-row xl:items-end justify-between gap-8 mb-12 border-b border-white/[0.05] pb-10 relative">
          ...
          <div>
            <Link 
              href="/perfil" 
              className="inline-flex items-center gap-2 text-neutral-500 hover:text-[#FDCB02] transition-colors text-[10px] font-black uppercase tracking-[0.3em] mb-6 group"
            >
            ...
            <h1 className="text-6xl md:text-8xl font-[1000] uppercase tracking-tighter leading-none text-transparent bg-clip-text bg-gradient-to-br from-white via-neutral-200 to-neutral-600">
              ¡Hola Jack!<span className="text-[#FDCB02]">.</span>
            </h1>
            <div className="mt-6 flex items-center gap-4">
              <span className="px-3 py-1 rounded-full bg-red-500/10 border border-red-500/20 text-red-500 text-[9px] font-black uppercase tracking-widest flex items-center gap-2 backdrop-blur-md shadow-lg">
                <span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse" /> Autorización: CEO
              </span>
              <span className="text-[10px] text-neutral-500 font-mono tracking-widest uppercase">
                Operación Activa • Coyote Textil
              </span>
            </div>
          </div>

          <div className="flex items-center gap-3">
             {/* El botón inteligente que soluciona el 404 */}
             <ExportButton />
          </div>
        </header>
          ...
          <div className="lg:col-span-8 bg-gradient-to-br from-[#0A0A0A] to-[#050505] border border-white/[0.05] p-8 md:p-12 rounded-[2.5rem] relative overflow-hidden group hover:border-white/10 transition-all duration-500">
            ...
            <div className="relative z-10 flex flex-col h-full justify-between">
              <div>
                ...
                <div className="flex items-center gap-3 text-xs text-neutral-500 font-bold uppercase tracking-widest bg-white/[0.03] w-fit px-4 py-2 rounded-full border border-white/5">
                  <TrendingUp size={14} className="text-green-500" /> Capital total auditado en base de datos
                </div>
              </div>
        ...
        <div className="bg-[#050505] border border-white/[0.05] rounded-[2.5rem] overflow-hidden shadow-2xl">
          <div className="p-8 md:p-10 border-b border-white/5 flex flex-wrap gap-6 items-center justify-between bg-white/[0.01]">
            ...
            <Link href="/admin/pedidos" className="text-[10px] text-[#FDCB02] hover:text-black hover:bg-[#FDCB02] font-black uppercase tracking-[0.3em] transition-all flex items-center gap-3 border border-[#FDCB02]/30 px-6 py-3 rounded-xl">
                  ...
                  <tr key={order.id} className="hover:bg-white/[0.02] transition-colors group">
                    ...
                    <td className="p-8">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="text-[11px] font-black text-white uppercase tracking-widest">{order.customerName}</p>
                          <p className="text-[9px] text-neutral-600 font-mono mt-1 italic">{order.customerEmail}</p>
                        </div>
                      </div>
                    </td>
                    <td className="p-8">
